...
) -> Generator[tuple[str, list[DatasetTestMetadata]], None, None]:
...
def generate_params_with_split_ratios(
    ...
    tuple[str, list[DatasetTestMetadata], list[float] | None, Exception | None],
    None,
    None,
]:
    for nb_dataset in nb_datasets:
...
def generate_params_for_nested_datasets(
    nb_datasets: list[int],
) -> Generator[tuple[str, list[DatasetTestMetadata], Exception], None, None]:
    for nb_dataset in nb_datasets:
        experiment_name = get_experiment_name(nb_dataset=nb_dataset)
        if nb_dataset == 1:
            dataset_metadatas = [
...
def check_splited_assets(
    ...
    assert len(dataset_collection["train"].assets) == nb_train_assets, (
        f"Incorrect number of train assets for {nb_dataset} dataset(s)"
    )
    assert len(dataset_collection["val"].assets) == nb_val_assets, (
        f"Incorrect number of val assets for {nb_dataset} dataset(s)"
    )
    assert len(dataset_collection["test"].assets) == nb_test_assets, (
...
class TestDatasetHandler:
    ...
    def test_get_split_ratios(
        ...
        experiment_name: str,
        datasets: list[DatasetTestMetadata],
        expected_split_ratios: list | None,
