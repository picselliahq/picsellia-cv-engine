# Model Version Processing Pipeline

This guide explains how to create, customize, test, and deploy a processing pipeline that runs on a **Model Version** using the `pxl-pipeline` cli with the `model_version` template.

These pipelines receive a model version as their target and can apply conversions, compressions, or any custom logic to the model.
...
```sh
pxl-pipeline init my_model_pipeline --type processing --template model_version
```
...
Contains the `process()` step where your core logic lives. The context gives you access to the target model version, parameters, and any inputs you've declared.
...
Fill in the `target_id` with the UUID of the model version you want to process.

## 4. Manage dependencies with `uv`

```bash
uv add onnx --project my_model_pipeline
```
...
```sh
pxl-pipeline test my_model_pipeline
```

This will:

- Prompt for the target model version if not set in the run config
...
```bash
pxl-pipeline test my_model_pipeline --reuse-dir
```
...
```sh
pxl-pipeline deploy my_model_pipeline
```
